...
export const createClass = asyncHandler(async (req, res) => {
  ...
  if (req.files?.["instructor.image"][0]?.buffer) {
    const { publicId, secureUrl } = await fileUpload(
      req.files["instructor.image"][0].buffer,
      "instructor"
    );
    newClass.instructor = {
      name: req.validatedData.body.instructorName,
...
export const getClasses = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    totalClassesResult.status !== "fulfilled"
  ) {
  ...
  sendResponse(
    ...
    "Total classes fetched successfully"
  );
});
...
export const getScheduledClass = asyncHandler(async (req, res) => {
  ...
  user.upcomingSchedule = user.upcomingSchedule.filter((schedule) => {
    ...
    if (schedule?.item?.status !== "scheduled") {
      return false;
    }
    ...
    if (
      ...
      (timeObj.hours < currentDate.getHours() ||
        (timeObj.hours === currentDate.getHours() &&
          timeObj.minutes <= currentDate.getMinutes()))
    ) {
      return false;
    }
  ...
  sendResponse(
    res,
    200,
    user.upcomingSchedule,
    "Scheduled classes fetched successfully"
  );
});
