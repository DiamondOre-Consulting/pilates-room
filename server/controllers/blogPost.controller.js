...
const getBlogPostBySlug = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, blogPost, ""));
});
...
const updateBlogPost = asyncHandler(async (req, res) => {
  ...
  const uploadImage = req.file
    ? await fileUpload(req?.file?.buffer, "blogImage", "blog")
    : null;
...
const getFeaturedBlogPosts = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, featuredPosts, ""));
});
