...
const getDashboardStats = asyncHandler(async (req, res) => {
  ...
  const recentMembers = await User.find({ isMember: true })
    ...
    .limit(5)
    .select(
      "firstName lastName email isMember isDiscovery memberShipPlan createdAt"
    )
    .lean();
...
const addMembershipPlanByAdmin = asyncHandler(async (req, res) => {
  ...
  if (!existingUser.isDiscovery) {
    ...
    existingUser.memberShipPlan.memberShipCount =
      existingUser.memberShipPlan.memberShipCount + 1;
