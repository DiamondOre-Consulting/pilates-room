...
import { getAllBlogs, deleteBlog, createBlog, editBlog } from '../Redux/Slices/blogSlice';
...
const BlogManagement = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            header: 'Status',
            cell: ({ row }) => (
                <span className={`capitalize ${row?.original?.status === 'published' ? 'text-green-600' :
                    row?.original?.status === 'draft' ? 'text-yellow-600' :
                        'text-gray-600'
                    }`}>
                    {row?.status}
                </span>
